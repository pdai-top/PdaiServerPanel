...
type BuildContext struct {
	...
	ImageTag   string // desired Docker image tag (e.g., "pdai-project-5:3")
...
func InstallCommand(builderType string) string {
	switch builderType {
	case "nixpacks":
		return "tmp=$(mktemp /tmp/nixpacks-install.XXXXXX.sh) && trap 'rm -f \"$tmp\"' EXIT && curl -fsSL https://nixpacks.com/install.sh -o \"$tmp\" && chmod +x \"$tmp\" && sh \"$tmp\""
	case "paketo":
		return "tmp=$(mktemp /tmp/pack.XXXXXX.tgz) && trap 'rm -f \"$tmp\"' EXIT && curl -fsSL 'https://github.com/buildpacks/pack/releases/latest/download/pack-v0.36.4-linux.tgz' -o \"$tmp\" && tar -xzf \"$tmp\" -C /usr/local/bin"
	case "railpack":
		return "tmp=$(mktemp /tmp/railpack-install.XXXXXX.sh) && trap 'rm -f \"$tmp\"' EXIT && curl -fsSL https://railpack.com/install.sh -o \"$tmp\" && chmod +x \"$tmp\" && sh \"$tmp\""
	default:
		return ""
	}
}
