...
// when PDAI_MAX_CONCURRENT_BUILDS isn't set. 3 is conservative for
...
type Service struct {
	...
	// the host. Capacity is `pdaiMaxConcurrentBuilds` env var,
...
//
//  1. PDAI_MAX_CONCURRENT_BUILDS env var (operator override; survives
...
func parseMaxConcurrentBuilds(logger *slog.Logger, coreAPI pluginpkg.CoreAPI) int {
	...
	if v := os.Getenv("PDAI_MAX_CONCURRENT_BUILDS"); v != "" {
		n, err := strconv.Atoi(v)
		if err != nil || n <= 0 {
			logger.Warn("invalid PDAI_MAX_CONCURRENT_BUILDS, using default",
