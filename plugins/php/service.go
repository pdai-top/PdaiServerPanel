...
func (s *Service) CreateRuntimeStream(req *CreateRuntimeRequest, progressCb func(string)) (*PHPRuntime, error) {
	...
	containerName := fmt.Sprintf("pdai-php-fpm-%s", strings.ReplaceAll(req.Version, ".", ""))
	...
	customImage := ""
	if len(req.Extensions) > 0 {
		customImage = fmt.Sprintf("pdai-php-fpm-%s:custom", strings.ReplaceAll(req.Version, ".", ""))
	}
	...
	if err := os.WriteFile(filepath.Join(rtDir, "conf.d", "99-pdai.ini"), []byte(iniContent), 0644); err != nil {
	...
	if err := os.WriteFile(filepath.Join(rtDir, "php-fpm.d", "zz-pdai.conf"), []byte(fpmContent), 0644); err != nil {
...
func (s *Service) UpdateConfig(id uint, req *UpdateConfigRequest) error {
	...
	iniPath := filepath.Join(rt.DataDir, "conf.d", "99-pdai.ini")
	confPath := filepath.Join(rt.DataDir, "php-fpm.d", "zz-pdai.conf")
...
func (s *Service) InstallExtensions(id uint, extensions []string, progressCb func(string)) error {
	...
	customImage := fmt.Sprintf("pdai-php-%s-%s:custom", rt.Type, strings.ReplaceAll(rt.Version, ".", ""))
...
func (s *Service) createFrankenSite(site *PHPSite, req *CreateSiteRequest, progressCb func(string)) (*PHPSite, error) {
	...
	site.Port = port
	site.ContainerName = fmt.Sprintf("pdai-fp-%s", sanitizeName(req.Name))
	...
	if err := os.WriteFile(filepath.Join(site.DataDir, "conf.d", "99-pdai.ini"), []byte(iniContent), 0644); err != nil {
		return nil, fmt.Errorf("write php.ini: %w", err)
	}
	...
	if len(req.Extensions) > 0 {
		...
		customImage = fmt.Sprintf("pdai-fp-%s:custom", sanitizeName(req.Name))
