...
// GetStatus returns the iptables status.
...
func (h *Handler) AddPort(c *gin.Context) {
	...
	if err := h.svc.AddPort(req.Zone, req.Port, req.Protocol, req.Comment); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
...
// UpdatePort updates a port rule.
func (h *Handler) UpdatePort(c *gin.Context) {
	...
	if err := h.svc.UpdatePort(req.Zone, req.OldPort, req.OldProtocol, req.Port, req.Protocol, req.Comment); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "port updated"})
}
...
// AvailableServices returns all known service shortcuts supported by the panel.
...
// ReloadFirewall reloads iptables rules where possible.
func (h *Handler) ReloadFirewall(c *gin.Context) {
	if err := h.svc.Reload(); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "iptables reloaded"})
}

// ClearRules removes non-essential iptables port rules and restores essentials.
func (h *Handler) ClearRules(c *gin.Context) {
	if err := h.svc.ClearRules(); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "iptables rules cleared"})
}

// StartFirewalld initializes iptables; the method name is kept for route compatibility.
func (h *Handler) StartFirewalld(c *gin.Context) {
	if err := h.svc.StartFirewalld(); err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	c.JSON(http.StatusOK, gin.H{"message": "iptables initialized"})
}

// InstallFirewalld streams the iptables installation progress via SSE.
