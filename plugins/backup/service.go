...
func (s *Service) backupPanel(parentCtx context.Context, destDir string, snapID uint) {
	...
	dbPath := os.Getenv("PDAI_DB_PATH")
	if dbPath == "" {
		dbPath = "/opt/pdai/data/pdai.db"
	}
	if _, err := os.Stat(dbPath); err == nil {
		vacuumDest := filepath.Join(panelDir, "pdai.db")
	...
	// Copy Caddyfile.
	caddyfilePath := os.Getenv("PDAI_CADDYFILE_PATH")
	if caddyfilePath == "" {
...
func (s *Service) backupDatabasesLegacy(parentCtx context.Context, destDir string, snapID uint) {
	...
	// Get running database containers (matching pdai-db-* naming).
	ctx, cancel := context.WithTimeout(parentCtx, 30*time.Second)
	defer cancel()
	cmd := exec.CommandContext(ctx, "docker", "ps", "--filter", "name=pdai-db-",
		"--format", "{{.Names}}")
	output, err := cmd.Output()
	if err != nil {
