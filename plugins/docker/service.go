...
func (s *Service) StackLogsFollow(ctx context.Context, id uint, tail string) (io.ReadCloser, error) {
	...
	cmdName, cmdArgs := composeCommand("logs", "--follow", "--tail", tail, "--no-color")
	...
	if err := cmd.Start(); err != nil && DetectRuntime() == RuntimePodman {
		if fallbackName, fallbackArgs, ok := podmanComposeCommand("logs", "--follow", "--tail", tail, "--no-color"); ok {
			cmd = exec.CommandContext(ctx, fallbackName, fallbackArgs...)
			cmd.Dir = stack.DataDir
			cmd.Env = append(os.Environ(), "COMPOSE_PROJECT_NAME="+sanitizeName(stack.Name))
			stdout, err = cmd.StdoutPipe()
			if err != nil {
...
// runCompose executes a compose command in the given directory.
...
func (s *Service) runCompose(name, dir string, args ...string) error {
	_, err := s.runComposeCombinedOutput(name, dir, args...)
	return err
}
...
func (s *Service) runComposeOutput(name, dir string, args ...string) (string, error) {
	output, err := s.runComposeCombinedOutput(name, dir, args...)
	if err != nil {
		return "", err
	}
...
func (s *Service) runComposeCombinedOutput(name, dir string, args ...string) ([]byte, error) {
	cmdName, cmdArgs := composeCommand(args...)
	output, err := runComposeCmd(cmdName, cmdArgs, name, dir)
	if err == nil || DetectRuntime() != RuntimePodman {
		return output, composeError(cmdName, args, output, err)
	}

	fallbackName, fallbackArgs, ok := podmanComposeCommand(args...)
	if !ok {
		return output, composeError(cmdName, args, output, err)
	}
	fallbackOutput, fallbackErr := runComposeCmd(fallbackName, fallbackArgs, name, dir)
	if fallbackErr == nil {
		return fallbackOutput, nil
	}

	s.logger.Error("compose command failed",
		"dir", dir,
		"args", strings.Join(args, " "),
		"output", strings.TrimSpace(string(fallbackOutput)),
		"err", fallbackErr,
	)
	return fallbackOutput, composeError(fallbackName, args, fallbackOutput, fallbackErr)
}

func runComposeCmd(cmdName string, cmdArgs []string, name, dir string) ([]byte, error) {
...
func composeCommand(args ...string) (string, []string) {
	return "docker", append([]string{"compose"}, args...)
}

func podmanComposeCommand(args ...string) (string, []string, bool) {
	if _, err := exec.LookPath("podman-compose"); err != nil {
		return "", nil, false
	}
	return "podman-compose", args, true
}

func composeError(cmdName string, args []string, output []byte, err error) error {
	if err == nil {
		return nil
	}
	outStr := strings.TrimSpace(string(output))
	if outStr != "" {
		return fmt.Errorf("%s %s: %s", cmdName, args[0], outStr)
	}
	return fmt.Errorf("%s %s: %v", cmdName, args[0], err)
}
