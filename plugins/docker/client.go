...
// NewClient creates a Client connected to the container runtime socket.
// It prefers an explicit socketPath, then DOCKER_HOST / CONTAINER_HOST,
// then falls back to a runtime-aware default.
...
func resolveSocketPath(socketPath string) string {
	if socketPath != "" {
		return trimUnixHost(socketPath)
	}
	if host := trimUnixHost(os.Getenv("DOCKER_HOST")); host != "" {
		return host
	}
	if host := trimUnixHost(os.Getenv("CONTAINER_HOST")); host != "" {
		return host
	}
...
func trimUnixHost(value string) string {
...
// ContainerDetail describes the editable subset of a standalone container.
...
// GetContainer returns a single container's editable configuration snapshot.
func (c *Client) GetContainer(ctx context.Context, id string) (*ContainerDetail, error) {
	inspect, err := c.cli.ContainerInspect(ctx, id)
	if err != nil {
		return nil, err
	}
	return containerDetailFromInspect(inspect), nil
}

// UpdateContainer recreates an existing container with updated settings.
// Docker does not support in-place editing for most of the fields this UI
// exposes, so the safest user-facing behavior is: inspect -> stop -> remove ->
// recreate with the requested configuration while preserving the name by
// default.
func (c *Client) UpdateContainer(ctx context.Context, id string, req *RunContainerRequest) (string, error) {
	inspect, err := c.cli.ContainerInspect(ctx, id)
	if err != nil {
		return "", err
	}
	current := containerDetailFromInspect(inspect)
	if current == nil {
...
func containerDetailFromInspect(inspect types.ContainerJSON) *ContainerDetail {
	if inspect.ID == "" {
		return nil
	}
	...
	if inspect.Config != nil {
		...
		detail.Env = append([]string(nil), inspect.Config.Env...)
	...
	if inspect.HostConfig != nil {
		...
		if detail.RestartPolicy == "" || detail.RestartPolicy == string(container.RestartPolicyDisabled) {
			detail.RestartPolicy = "no"
		}
		detail.MemoryLimit = inspect.HostConfig.Resources.Memory
		if inspect.HostConfig.Resources.NanoCPUs > 0 {
			detail.CPULimit = float64(inspect.HostConfig.Resources.NanoCPUs) / 1e9
		}
		for port, bindings := range inspect.HostConfig.PortBindings {
			...
			if len(bindings) == 0 {
				detail.Ports = append(detail.Ports, RunPortMapping{ContainerPort: containerPort, Protocol: protocol})
				continue
			}
	...
	for _, mount := range inspect.Mounts {
		...
		detail.Volumes = append(detail.Volumes, RunVolumeMapping{
