...
func (s *HostService) List(filters ...HostListFilter) ([]model.Host, error) {
	...
	query := s.db.Preload("Domains", func(db *gorm.DB) *gorm.DB { return db.Order("sort_order ASC") }).
		Preload("Upstreams").Preload("CustomHeaders").Preload("AccessRules").Preload("Routes").Preload("BasicAuths").
...
func (s *HostService) Get(id uint) (*model.Host, error) {
	var host model.Host
	err := s.db.Preload("Domains", func(db *gorm.DB) *gorm.DB { return db.Order("sort_order ASC") }).
		Preload("Upstreams").Preload("CustomHeaders").Preload("AccessRules").Preload("Routes").Preload("BasicAuths").
		Preload("Group").Preload("Tags").
		First(&host, id).Error
	if err != nil {
...
func (s *HostService) Update(id uint, req *model.HostCreateRequest) (*model.Host, error) {
	...
	host.FullCaddyBlock = strings.TrimSpace(req.FullCaddyBlock)
	...
	s.db.Where("host_id = ?", id).Delete(&model.HostDomain{})
	...
	host.Domains = buildHostDomainRows(id, domains[1:])
	host.Upstreams = nil
	...
	if err := s.db.Omit("Domains", "Upstreams", "CustomHeaders", "AccessRules", "Routes", "BasicAuths", "Tags").Save(host).Error; err != nil {
		return nil, fmt.Errorf("failed to update host: %w", err)
	}

	// Explicitly clear group_id if nil (GORM Save ignores nil pointer fields)
	if req.GroupID == nil {
		if err := s.db.Model(&model.Host{}).Where("id = ?", id).Update("group_id", nil).Error; err != nil {
			return nil, fmt.Errorf("failed to clear host group: %w", err)
		}
	}

	for i := range host.Domains {
		host.Domains[i].ID = 0
		host.Domains[i].HostID = id
		if err := s.db.Create(&host.Domains[i]).Error; err != nil {
			return nil, fmt.Errorf("failed to update host domain: %w", err)
		}
	}
	for i := range host.Upstreams {
		host.Upstreams[i].ID = 0
		host.Upstreams[i].HostID = id
		if err := s.db.Create(&host.Upstreams[i]).Error; err != nil {
	...
	if len(oldUpstreams) > 0 && len(host.Upstreams) > 0 {
		...
		if err := s.db.Where("host_id = ?", id).Find(&routes).Error; err != nil {
			return nil, fmt.Errorf("failed to load routes for upstream remap: %w", err)
		}
		for _, r := range routes {
			if r.UpstreamID != nil {
				if idx, ok := oldIDMap[*r.UpstreamID]; ok && idx < len(host.Upstreams) {
					newID := host.Upstreams[idx].ID
					if err := s.db.Model(&r).Update("upstream_id", newID).Error; err != nil {
						return nil, fmt.Errorf("failed to remap route upstream: %w", err)
					}
				}
			}
		}
	...
	for i := range host.CustomHeaders {
		...
		host.CustomHeaders[i].HostID = id
		if err := s.db.Create(&host.CustomHeaders[i]).Error; err != nil {
			return nil, fmt.Errorf("failed to update custom header: %w", err)
		}
	}
	for i := range host.AccessRules {
		host.AccessRules[i].ID = 0
		host.AccessRules[i].HostID = id
		if err := s.db.Create(&host.AccessRules[i]).Error; err != nil {
			return nil, fmt.Errorf("failed to update access rule: %w", err)
		}
	}
	for i := range host.BasicAuths {
		host.BasicAuths[i].ID = 0
		host.BasicAuths[i].HostID = id
		if err := s.db.Create(&host.BasicAuths[i]).Error; err != nil {
			return nil, fmt.Errorf("failed to update basic auth: %w", err)
		}
	}

	// Sync tag associations: replace all
	if err := s.db.Where("host_id = ?", id).Delete(&model.HostTag{}).Error; err != nil {
		return nil, fmt.Errorf("failed to clear host tags: %w", err)
	}
	for _, tagID := range req.TagIDs {
		if err := s.db.Create(&model.HostTag{HostID: id, TagID: tagID}).Error; err != nil {
			return nil, fmt.Errorf("failed to update host tag: %w", err)
		}
	}
...
func (s *HostService) ImportAll(data *model.ExportData) error {
	...
	for _, host := range data.Hosts {
		...
		for _, d := range host.Domains {
			if err := caddy.ValidateDomain(d.Domain); err != nil {
				return fmt.Errorf("import validation failed for additional domain '%s' on '%s': %w", d.Domain, host.Domain, err)
			}
		}
		allDomains := []string{host.Domain}
		for _, d := range host.Domains {
			allDomains = append(allDomains, d.Domain)
		}
		if err := caddy.ValidateFullCaddyBlockForDomains(allDomains, host.FullCaddyBlock); err != nil {
			return fmt.Errorf("import validation failed for full Caddy block on '%s': %w", host.Domain, err)
		}
...
func (s *HostService) CloneHost(sourceID uint, newDomain string) (*model.Host, error) {
	...
	txErr := s.db.Transaction(func(tx *gorm.DB) error {
		...
		newHost = &model.Host{
			...
			FullCaddyBlock:   source.FullCaddyBlock,
...
func normalizeHostDomains(primary string, extras []string) (string, []string, error) {
	primary = strings.ToLower(strings.TrimSpace(primary))
	if err := caddy.ValidateDomain(primary); err != nil {
		return "", nil, err
	}
...
func hostDomainsFromRequest(req *model.HostCreateRequest) ([]string, error) {
	primary, extras, err := normalizeHostDomains(req.Domain, req.Domains)
	if err != nil {
		return nil, err
	}
	return append([]string{primary}, extras...), nil
}

func loadHostDomains(host *model.Host) []string {
	domains := []string{host.Domain}
	for _, d := range host.Domains {
		if name := strings.TrimSpace(d.Domain); name != "" && !strings.EqualFold(name, host.Domain) {
...
func checkDomainConflicts(db *gorm.DB, hostID *uint, domains []string) error {
	if len(domains) == 0 {
		return nil
	}
	for _, domain := range domains {
		var count int64
		query := db.Model(&model.Host{}).Where("LOWER(domain) = ?", strings.ToLower(domain))
		if hostID != nil {
			query = query.Where("id != ?", *hostID)
		}
		...
		query = db.Model(&model.HostDomain{}).Where("LOWER(domain) = ?", strings.ToLower(domain))
		if hostID != nil {
...
func buildHostDomainRows(hostID uint, domains []string) []model.HostDomain {
	rows := make([]model.HostDomain, 0, len(domains))
