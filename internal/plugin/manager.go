...
func (m *Manager) InitAll() error {
	...
	// 4. Apply guard middleware to all configured router groups BEFORE creating
	// sub-groups. In Gin, child groups copy the parent's handler chain at
	// creation time, so the guard must be added before any Group() calls.
	guard := m.PluginGuardMiddleware()
	if m.router != nil {
		m.router.Use(guard)
	}
	if m.operatorRouter != nil {
		m.operatorRouter.Use(guard)
	}
	if m.adminRouter != nil {
		m.adminRouter.Use(guard)
	}
	if m.publicRouter != nil {
	...
	for _, id := range m.order {
		...
		// Create sub-routers under /api/plugins/{id}. Public routes are optional
		// because trimmed builds may not expose public plugin endpoints.
		var pluginRouter, operatorPluginRouter, adminPluginRouter, publicPluginRouter *gin.RouterGroup
		if m.router != nil {
...
// HaisPalen keeps infrastructure plugins opt-in by default while existing
// installs keep their persisted state.
...
// /api/plugins/{id}/... Returns empty string for plugin management routes.
func extractPluginID(path string) string {
	...
	for i, p := range parts {
		...
		id := parts[i+1]
		if id == "frontend-manifests" {
			return ""
		}
		if i+2 < len(parts) {
			switch parts[i+2] {
			case "enable", "disable", "sidebar", "install":
				return ""
			}
		}
		return id
