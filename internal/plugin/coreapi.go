...
import (
	...
	"github.com/pdai/pdai/internal/crypto"
...
func (a *CoreAPIImpl) firewallDefaultZone() string { return "iptables" }

func (a *CoreAPIImpl) FirewallStatus() (map[string]interface{}, error) {
	result := map[string]interface{}{"installed": false, "running": false, "default_zone": "iptables", "zones": []string{"iptables"}}
	if _, err := exec.LookPath("iptables"); err != nil {
		return result, nil
	}
	result["installed"] = true
	if _, err := a.firewallCmd("-S", "INPUT"); err != nil {
		return result, nil
	}
	result["running"] = true
	...
	result["active_zones"] = []map[string]interface{}{{"name": "iptables", "active": true, "services": []string{}, "ports": []string{}}}
	return result, nil
}
...
func (a *CoreAPIImpl) FirewallListRules(zone string) (map[string]interface{}, error) {
	...
	if zone != "iptables" {
		return nil, fmt.Errorf("iptables panel does not support zones")
	}
	out, err := a.firewallCmd("-S", "INPUT")
	if err != nil {
		return nil, fmt.Errorf("list iptables rules: %w", err)
	}

	result := map[string]interface{}{"zone": zone, "services": []string{}, "ports": []string{}, "interfaces": []string{}, "rich_rules": []string{}}
	var ports []string
	seen := map[string]bool{}
	for _, line := range strings.Split(out, "\n") {
		line = strings.TrimSpace(line)
		if !strings.HasPrefix(line, "-A INPUT ") || !strings.Contains(line, " -j ACCEPT") {
			continue
		}
		port, ok := parseIPTablesPortRule(line)
		if ok && !seen[port] {
	...
	result["ports"] = ports
	result["target"] = "iptables INPUT ACCEPT rules"
	return result, nil
}
...
var (
	...
	fwZoneRe     = regexp.MustCompile(`^iptables$`)
...
func (a *CoreAPIImpl) FirewallAddPort(zone, port, protocol string) error {
	...
	if zone != "iptables" {
		return fmt.Errorf("iptables panel does not support zones")
	}
	args := append([]string{"-C", "INPUT"}, iptablesPortRuleArgs(port, protocol)...)
	if _, err := a.firewallCmd(args...); err == nil {
		return nil
	}
	args = append([]string{"-I", "INPUT", "1"}, iptablesPortRuleArgs(port, protocol)...)
	if _, err := a.firewallCmd(args...); err != nil {
...
func (a *CoreAPIImpl) FirewallRemovePort(zone, port, protocol string) error {
	...
	args := append([]string{"-D", "INPUT"}, iptablesPortRuleArgs(port, protocol)...)
	if _, err := a.firewallCmd(args...); err != nil {
...
func (a *CoreAPIImpl) FirewallAddService(zone, service string) error {
	...
	case "ssh", "openssh":
		return a.FirewallAddPort(zone, "22", "tcp")
	case "http":
		return a.FirewallAddPort(zone, "80", "tcp")
	case "https":
		return a.FirewallAddPort(zone, "443", "tcp")
	default:
		return fmt.Errorf("iptables panel only supports built-in services: ssh, http, https")
	}
}
...
func (a *CoreAPIImpl) FirewallRemoveService(zone, service string) error {
	...
	case "http":
		return a.FirewallRemovePort(zone, "80", "tcp")
	case "https":
		return a.FirewallRemovePort(zone, "443", "tcp")
	default:
		return fmt.Errorf("iptables panel only supports built-in services: ssh, http, https")
	}
}

func iptablesPortRuleArgs(port, protocol string) []string {
	args := []string{"-p", protocol}
	if strings.Contains(port, "-") {
		args = append(args, "-m", "multiport", "--dports", strings.ReplaceAll(port, "-", ":"))
	} else {
...
func parseIPTablesPortRule(line string) (string, bool) {
	...
	if protocol == "" || port == "" || !fwProtocolRe.MatchString(protocol) || !fwPortRe.MatchString(port) {
		return "", false
	}
