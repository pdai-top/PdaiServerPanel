...
// library defaults for Pdai's use cases. Specifically, every long-running
...
// would need a build-tagged fallback. Pdai targets EL9/EL10 so that is
// intentional.
...
// KillProcessGroup force-kills the process group owned by cmd. It is safe to
// call after CommandContext cancellation as a second best-effort cleanup path.
func KillProcessGroup(cmd *exec.Cmd) error {
	if cmd == nil || cmd.Process == nil {
		return nil
	}
	return KillProcessGroupPID(cmd.Process.Pid)
}

// KillProcessGroupPID force-kills the process group identified by pid.
