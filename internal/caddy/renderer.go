...
func RenderCaddyfile(hosts []model.Host, cfg *config.Config, dnsProviders map[uint]model.DnsProvider) string {
	...
	// Header
	b.WriteString("# ============================================\n")
	b.WriteString("# Auto-generated by Pdai\n")
...
func hostSiteDomains(host model.Host) []string {
	domains := []string{host.Domain}
	additional := make([]model.HostDomain, len(host.Domains))
	copy(additional, host.Domains)
	sort.Slice(additional, func(i, j int) bool {
		return additional[i].SortOrder < additional[j].SortOrder
	})

	seen := map[string]bool{strings.ToLower(strings.TrimSpace(host.Domain)): true}
...
func renderHostBlock(b *strings.Builder, host model.Host, cfg *config.Config, dnsProviders map[uint]model.DnsProvider) {
	if strings.TrimSpace(host.FullCaddyBlock) != "" {
		b.WriteString(strings.TrimSpace(host.FullCaddyBlock))
		b.WriteString("\n\n")
		return
	}

	// Domain line
	domains := hostSiteDomains(host)
