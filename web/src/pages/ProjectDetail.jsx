...
export default function ProjectDetail() {
    ...
    const fetchProject = async () => {
        try {
            ...
            setStartCommand(res.data.start_command || '')
    ...
    const saveStartCommand = async () => {
        setSaving(true)
        try {
            await deployAPI.updateProject(id, { start_command: startCommand.trim() })
            fetchProject()
        } catch (e) {
            alert(e.response?.data?.error || t('common.save_failed'))
        } finally {
            ...
            <Tabs.Root defaultValue="logs">
                ...
                <Tabs.Content value="env">
                    <Card mt="3">
                        ...
                        <Box mb="4">
                            <Text size="2" weight="medium" mb="1" as="div">{t('deploy.start_command')}</Text>
                            <Flex gap="2" align="center">
                                ...
                                />
                                <Button size="1" onClick={saveStartCommand} disabled={saving}>
                                    {saving ? t('common.saving') : t('common.save')}
                                </Button>
                            </Flex>
                        </Box>
